...
import (
	...
	"extension-scaffold/tools/pkg/contracts/latch"
...
func DeployInstructionSender(s *support.Support) (common.Address, *latch.LatchInstructionSender, error) {
	...
	address, tx, contract, err := latch.DeployLatchInstructionSender(
...
func SetExtensionId(s *support.Support, instructionSenderAddress common.Address) error {
	sender, err := latch.NewLatchInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	if err != nil {
		...
		if reason == "" {
			parsed, _ := latch.LatchInstructionSenderMetaData.GetAbi()
			if parsed != nil {
...
// SendCancelIntent publicly revokes an armed intent. LATCH's arm/status path
// is a direct action and never comes through the chain; the exits do.
func SendCancelIntent(s *support.Support, instructionSenderAddress common.Address, commitment [32]byte) (common.Hash, common.Hash, error) {
	sender, err := latch.NewLatchInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	tx, err := sender.CancelIntent(opts, commitment)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := latch.LatchInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("cancelIntent", commitment)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
